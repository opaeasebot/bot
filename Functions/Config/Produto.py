...
def GerarPainelProduto(inter: disnake.MessageInteraction | None, produtoID: str, bot: commands.Bot | None = None):
    ...
    if len(produto["campos"]) > 1:
        ...
        select = [
            disnake.ui.Button(label="Comprar", emoji="<:cart_emoji:1240652524646830090>", custom_id=f"ComprarProduto_{produtoID}_{campoID}")
        ] # coloquei aqui o emoji de carrinho da ease
    else:
        embed.add_field(name="Valor à vista", value=f"``R$--``", inline=True)
        embed.add_field(name="Restam", value=f"``--``", inline=True)
        select = [
            disnake.ui.Button(label="Comprar", emoji="<:cart_emoji:1240652524646830090>", custom_id=f"null", disabled=True)
        ] # coloquei aqui o emoji de carrinho da ease   
